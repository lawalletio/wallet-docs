...
Upon successfully parsing and validating the request body, the asynchronous NOSTR API endpoint (ie. `{base API url}/nostr/publish`) will respond with:
...
On the other hand, upon successfully parsing and validating the request body, the synchronous NOSTR API endpoint (ie. `{base API url}/nostr/dispatch`) will respond with:
